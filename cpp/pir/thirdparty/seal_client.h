...
class PIRClient {
public:
    PIRClient(int number_of_items, int size_per_item);

    void GenerateKey(std::vector<uint8_t> &secret_key, std::vector<uint8_t> &galois_key);

    void GenerateQuery(std::vector<uint8_t> &qurey, int index);

    void loadKey(const std::vector<uint8_t> &secret_key);
...
//    seal::Plaintext decode_reply(PirReply &reply);
    ...
    PirReply deserialize_reply(const std::vector<uint8_t> &reply);
