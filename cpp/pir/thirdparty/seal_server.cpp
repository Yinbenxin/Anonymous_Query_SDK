...
PIRServer::PIRServer(int number_of_items, int size_per_item) {
...
void PIRServer::set_galois_key(const vector<uint8_t>&galois_keys) {

    std::stringstream galois_stream;
    galois_stream.write((char*)galois_keys.data(), galois_keys.size());
    GaloisKeys galkey;
    galkey.load(*context_, galois_stream);
    galoisKeys_ = galkey;
}

PirQuery PIRServer::deserialize_query(const std::vector<uint8_t>& query) {
  PirQuery q;
  std::stringstream stream;
  stream.write((char*)query.data(), query.size());
...
int PIRServer::serialize_reply(PirReply &reply, std::vector<uint8_t>&reply_output) {
    int output_size = 0;
    std::stringstream stream;
    ...
    reply_output.assign((std::istreambuf_iterator<char>(stream)),
...
int PIRServer::generate_reply(const std::vector<uint8_t> &query,
                   const std::vector<std::vector<uint8_t>> &data,
                   std::vector<uint8_t> &reply,
                   std::uint64_t ele_num, std::uint64_t ele_size){
    PirQuery q = deserialize_query(query);
    auto db(make_unique<uint8_t[]>(ele_num * ele_size));

    std::vector<uint8_t> temp(ele_size, 32); // 创建一个长度为 ele_size 的全零 std::vector<uint8_t>
    for (auto i = 0; i < ele_num; i++) {
        std::copy(data[i].begin(), data[i].end(), db.get() + ele_size*i); // 将 data[i] 中的数据复制到 temp 中
        std::copy(temp.begin(), temp.begin()+(ele_size-data[i].size()), db.get() + ele_size*i + data[i].size()); // 使用空格填充 temp 中剩余的空间
    }
    std::unique_ptr<Database>  dbs;
    set_data(std::move(db), dbs, ele_num, ele_size);
    PirReply r = generate_reply(q, dbs);
    int output_size = serialize_reply(r, reply);

    return 0;
}
void PIRServer::set_data(const std::unique_ptr<const uint8_t[]> &bytes,
                         std::unique_ptr<Database> & db,
                         uint64_t ele_num, uint64_t ele_size) {
    ...
    db = std::move(result);
//    auto evaluator1 = make_unique<Evaluator>(*context_);
    for (uint32_t i = 0; i < db->size(); i++) {
        evaluator_->transform_to_ntt_inplace(db->operator[](i), context_->first_parms_id());
    }
